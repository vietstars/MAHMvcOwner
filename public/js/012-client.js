...
;(function ($, window, undefined) { 
    $(document).ready(function () {
        ...
        $('.too-big,.type-filter').slimScroll({
            height: 350,
            railVisible: false,
            alwaysVisible: false,
            color:'#26a69a',
        ...
        $(document).on('click','.fa.fa-edit',function(){
            var _check=confirm('Do you want to change this text?');
            var _id=$(this).data('id');
            var _content=$(this).parent();
            var _val=_content.children().first().val();
            if(_check){
                $.post(URL+'editText',{text:_val,id:_id})
                toastr.clear();
                toastr.options.closeButton = true;
                toastr.options.positionClass='toast-top-right';
                toastr.success('You have been updated successfully!', '');
            }
        })
    })
})(jQuery, window);