...
;(function ($, window, undefined) { 
    $(document).ready(function () {
        $(".fixed-action-btn").actionBtn({layout:'vertizontal'});
        var footerHeight = $('#menubar .menubar-foot-panel').outerHeight();
        footerHeight = Math.max(footerHeight, 1);
        $('.menubar-scroll-panel').css({
            'padding-bottom': footerHeight
        ...
        $('.position-toast').on('click', function (e) {
            ...
            toastr.options.positionClass='toast-top-center'; //toast-top-left//toast-top-full-width//toast-top-right//toast-bottom-center//toast-bottom-full-width
            toastr.success('Position message', '');//error//warning//success//info
        });
        $(document).on('click',function(e){
            if(!$(e.target).closest('section.notify.container').length) {
                if($('section.notify.container').is(":visible")) {
                    $('section.notify.container').slideUp('slow',function(){$(this).remove()});
                }
            } 
        })
    })
