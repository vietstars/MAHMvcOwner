;(function ($, window, undefined) {	
    $(document).ready(function () {
		var LG=$("body").attr("lg"),URL=$("body").attr("url")+'admin/',CTL=$("body").attr("ctl"),ACT=$("body").attr("act");		
	  	$(".show-menu").ShowApi({
	  		url:URL+CTL+'/allMenu',
	  		layout:'<div class="col-xs-12 menu-form">'+
	  		'<div class="input-group input-group-sm">'+
                '<input type="text" data-input="menu" name="menu" placeholder="menu" class="form-control first-item">'+
                '<input type="text" data-input="controller" name="controller" placeholder="controller" class="form-control second-item">'+
                '<input type="text" data-input="action" name="action" placeholder="action" class="form-control third-item">'+
                '<input type="text" data-input="icon" name="icon" data-modal="open" data-target="#getIcons" placeholder="icon" class="form-control fourth-item">'+
                '<select class="form-control fiveth-item" name="parents"><option value=0>root</option></select>'+                
                '<input type="text" data-input="sorted" name="sorted" placeholder="sort" class="form-control sixth-item">'+
            ...
            '</div>',
	  		...
	  		editor:{
	  			...
	  			parent:"parent",
	  			sorted:"sorted",
	  			'select.fiveth-item':'current'/*URL+CTL+'/allMenu'*/,
	  			/*lock:{
	  				items:[".btn-remove"],
	  				key:["template","icon"]
	  			}*/
        ...
        $("#new-menuItem").on("click",function () {
	  			...
	  			if(question){
	  				...
	  				$.each(checked,function () {
						...
						$(this).prop('checked',false);
	  					...
	  					_origin.addClass('locked').find("input").not('[type="checkbox"]').attr("disabled","disabled");
	  		...
	  		if(checked.length){
	  			...
	  			if(question){
	  				...
	  				$.each(checked,function () {
	  					...
	  					_origin.removeClass('locked').find("input").removeAttr("disabled");
	  	...
	  	$(document).on("click",".btn-lock",function(){
	  		...
	  		if(question){
		  		...
		  		origin.addClass('locked').find("input").attr("readonly","readonly");
	  	...
	  	$(document).on("click",".btn-unlock",function(){
	  		...
	  		if(question){
		  		...
		  		origin.removeClass('locked').find("input").removeAttr("readonly");
	  	...
	  	$(".show-menu").on("change","input[type='text'],select",function () {
	  		$(this).siblings(".input-group-addon").find("input[type='checkbox']").prop('checked',true);
	  	})
