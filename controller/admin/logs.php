<?php
class adm_logs extends controller
{
	...
	{
		$this->view->title='Transaction Logs';
	}
			...
			if(file_exists($log)){
				...
				foreach ($logs as $key=>$item) {	
					...
					$data=array(				
						"ip"=>$item[1],
						"visitor"=>$visitor,
						"browser"=>$item[3]."<small>[".$item[4]."]</small>",	
						"os"=>$item[5],		
						"url"=>$item[6],
	...
	public function delLogs()
	{
		if($_POST['folder']){
			...
			$log=APP.'log/'.$_POST['folder'].'/log.csv';
			if(file_exists($log)){
				unlink($log);
