<?php
class adm_menu extends controller
{
	...
	{
		$result=$this->model->table('menu as `m` LEFT JOIN '.DB.N.'menu as `p` ON `m`.parents=`p`.id')->get("`m`.id,`m`.menu,`p`.menu`parent`,`m`.controller,`m`.action,`m`.icon,`m`.icon_ctl,`m`.sorted,`m`.ctl")->where("`m`.type=0")->order("`m`.ctl ASC,`m`.parents ASC,`m`.sorted ASC")->ctl('m')->multi()->select();
		foreach ($result as $k=>$v) {
			$result[$k]->menu=htmlspecialchars_decode($v->menu);
		}
		$this->api($result);
	}
	...
	{
		if($_POST['id']){
			if(isset($_POST['menu']))$_POST['menu']=htmlspecialchars($_POST['menu']);
