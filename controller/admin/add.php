<?php
class adm_add extends controller
{
	...
	{
		$this->view->title='Additional record Logs';
	}
		...
		if($_POST['date']){
			...
			if(file_exists($log)){
				...
				foreach ($logs as $key=>$item) {	
					...
					if(empty($item[2]))$visitor='Unknown';else{
	...
	public function delLogs()
	{
		if($_POST['folder']){
			...
			$log=APP.'log/'.$_POST['folder'].'/add.csv';
			if(file_exists($log)){
				unlink($log);
